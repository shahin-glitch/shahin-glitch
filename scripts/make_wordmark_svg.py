...
"""
Render "shahin-glitch" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
if os.name == "nt":
    ...
    DEFAULT_FONT = "/System/Library/Fonts/Futura.ttc"
FONT_PATH = os.environ.get("WORDMARK_FONT", DEFAULT_FONT)
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 0))   # .ttc collections use >0 but plain .ttf/.otf use 0
...
TEXT = os.environ.get("WORDMARK_TEXT", "Shahin")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">shahin@github: ~$ ./wordmark.sh --3d</text>')
